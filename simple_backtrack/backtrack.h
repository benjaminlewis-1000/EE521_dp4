...
void backtrack(std::vector<subsetContainer> subsetFull, 
        ...
        int universeSize, int &minSize, std::vector<int> &bestSolution){
	...
	//	std::cout << "Here" << endl;
		...
		//	std::cout << "It. remainging  " << iterationsRemaining << std::endl;
			int bestCase = 0;
			for (int k = 0; k < iterationsRemaining; k++){
				bestCase += subsetsCopy[k].numUncoveredElements;
			}
			
		//	std::cout << "Best case " << bestCase << " and uncovered " << uncoveredRemaining << std::endl;
		...
		/*	double scaleFactor = 1.0;
			...
			/*	bool combination_not_covered;
				...
				//if ( preceeding_not_valid || combination_not_covered ){
					backtrack(subsetFull, subsetsCopy, subsets_decimal, solutionCopy, solProgC, universeSize, minSize, bestSolution);
			/*	}else{
					std::cout << "Pruned " << thisRound << " has already been preceeded by " << preceeding << std::endl;
				}
				if (preceeding != -1){
